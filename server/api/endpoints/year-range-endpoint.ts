...
export function getYearRange(req: Request, res: Response, next: NextFunction) {
    if(!req.body.startYear || !req.body.endYear) {
        ...
        const query = "SELECT * FROM movies_meta LEFT OUTER JOIN movie_posters ON (title = poster_title AND movie_id = poster_id) WHERE EXTRACT (YEAR FROM release_date) BETWEEN $1 AND $2 "
